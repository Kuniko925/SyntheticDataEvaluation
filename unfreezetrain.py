...
def finetune_layerwise(model_class, conf, real_raito=0.1):
    ...
    def preparation(model_name, real_raito):
        # Preparation
        model_save_directory = config.PROJECT_ROOT / f"Unfreeze/{model_name}/{str(real_raito*10)}/" # e.g., layerwise/MobileNet/10/
    ...
    def testing(model_name, test_setting, trainer, model, test_loader, df_test):
        ...
        df_test['preds'] = preds
        df_test.to_csv(config.PROJECT_ROOT / f'results/{model_name}_unfreeze_{str(real_raito*10)}_{test_setting}.csv', index=False) # e.g., results/MobileNet_FAKE_REAL.csv/
